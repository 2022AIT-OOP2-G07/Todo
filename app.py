...
def todo_db():
  ...
  # ２．テーブル作成
  con.execute("CREATE TABLE IF NOT EXISTS todo(id integer PRIMARY KEY AUTOINCREMENT, todo_data text, todo_deadline datetime)")
  con.commit()
  # 3．データ参照
...
def add_todo():
    ...
    con = sqlite3.connect('todo_list.db')
    #データベースにデータを追加
    con.execute("INSERT INTO todo(todo_data, todo_deadline)values(?,?)",[add_todo, add_limit])
    con.commit()
    cur = con.execute("SELECT * FROM todo")
    cur = con.execute("select * from todo order by todo_deadline")
    data = cur.fetchall()
