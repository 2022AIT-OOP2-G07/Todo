...
def getData(self):
    global data, reloade, tuuti_state, notification, c, app_flag, task_flag, con, cur
...
def trigar(t):  # 通知が実行されている時に毎秒実行されるメソッド
    ...
    siteizikan_date = datetime.datetime.strptime(siteizikan, '%Y-%m-%d %H:%M')
...
#         self.menu[doc[1]].add(
#             MenuItem("削除", callback=delete_task(doc[0])))
#     # self.menu[doc[1]].add(MenuItem("削除"))
#     # for doc in cur:
#     # self.menu[doc[1]].add(
#     #     MenuItem("削除", callback=self.delete_task(doc[0])))
...
class MenuBar(rumps.App):
    ...
    def __init__(self):
        global data, notification, tuuti_state, app_flag, con, cur

        # server_run = app.app.run()
        ...
        super(MenuBar, self).__init__(name="メニューバーtodo", title=hyouzi + '×' if notification ==
                                      False else hyouzi+'⚪︎', icon='static/img/icon/icon.png', quit_button="終了")
        print("\n|\n|\n🟥チェック完了\n|\n|\nアプリ起動-no problem\n|\n|")
        app_flag = True
    ...
    #         print(doc)
    #         self.menu = MenuItem(doc[1])
    #         # self.menu[doc[1]].add(MenuItem("削除", callback=delete_task(doc[0])))
    #     # self.menu[doc[1]].add(MenuItem("削除"))
    #     # for doc in cur:
    #     # self.menu[doc[1]].add(
    #     #     MenuItem("削除", callback=self.delete_task(doc[0])))
